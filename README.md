...
**Team chat** lets the two players compare evidence and discuss their ending without a microphone or another call app. Open it from the bottom-right of the case; new partner messages show an unread count. Convex stores the latest 100 messages with the room, so reloading keeps the conversation. Messages are limited to 500 characters, sender attribution comes from the authenticated seat, and repeated send attempts reuse the same message. Only the joined players can read or write; per-player burst limits keep a partner from flooding the chat. Team chat is separate from Mara's AI conversation and is never included in model prompts or email debriefs. Optional voice follows the clue workspace so the first puzzle appears sooner on a small screen.

Every stage requires both players to share their own evidence. Room-specific ciphers and relay logs support replay. The case library can switch the opening chronology assignment between the authored first-departure brief and a validated later-departure pack; it does not generate testimony, endings or answer keys. Hints cost 10 team points and wrong answers cost 5, with a floor of zero. The score is a local team challenge, not a ranked leaderboard. A fresh case creates a new invitation and retains the five-room daily guest limit.
...
| Firecrawl | Optional fixed-URL checks extract both NASA mission pages, validate the pinned launch dates, and retain a short citation, extraction hash and timestamp. A fresh public receipt can feed the case compiler. Real development and production source calls passed. No arbitrary URL or crawl input. |
| OpenAI / Convex Agent | Mara Vale, a labelled fictional AI archivist, answers from shared clues and accepted room-local conversation. The same pinned OpenAI model may draft the bounded title, routing note, prompt and hint for one additional case pack; it never writes the answer or canonical story. Filtered development and production inference passed using `openai/gpt-4o-mini` through Vercel AI Gateway, restricted to OpenAI. No model fallback. |
...
Each room allows two source-check attempts (two fixed NASA pages per check) and six AI attempts. Shared deployment caps are ten source-check jobs, forty AI attempts and ten case-email jobs per daily window. Case-pack generation consumes that same forty-turn AI budget; it has no separate allowance. In the September 20 development build, recipient verification and debriefs share the ten email slots. Password recovery retains its separate existing limits. Failed attempts count. These are job limits, not a currency budget: Firecrawl's component may retry transient transport failures on the same provider. AI requests have a 45-second timeout, bounded output and zero SDK retries. No limits or provider keys were increased or changed.

The development build reuses validated public NASA receipts for up to 24 hours across rooms, preserving the original check time. It caches no player data or conversation. Concurrent uncached requests can still consume separate jobs. Sources are pinned once successful and cannot be refreshed into different facts mid-case. Missing or conflicting source facts produce an error, not a synthetic receipt.
...
The local suites now contain 91 tests, covering projection boundaries, membership, input and deployment limits, immutable pins, stale completion, room-local Agent history, withheld AI output, email prerequisites, generated-pack publication and server-computed answers. Narrative tests cover earned passages, sealed or expired rooms, legacy cases and both ending consequences. Fixture-model tests check prompt inclusion and response publication without calling a provider. Five team-chat tests cover two-seat history, membership, duplicate retries, sender limits and retention without changing game state. Live checks are documented separately in `hackathon.md` and the September 20 verification note.
...
The development build lets guests verify a private debrief recipient after completing a case, without setting up password recovery. Verification and sending require separate consent. The code expires after 15 minutes, is stored as a digest and works once; a room member cannot read their partner's address. The existing per-player, per-case send record prevents duplicate debriefs. September 20's controlled test received both emails in the authorized Gmail inbox.

Remaining: independent human playtesting. The [three-pair blind-trial protocol](PLAYTEST-PROTOCOL.md) is prepared; all trials remain pending. A gameplay-changing inbound character exchange remains a future feature. Launch copy, storyboards and recordings stay local and excluded from this repository.

Historical launch dates were checked against NASA's [Voyager 1](https://science.nasa.gov/mission/voyager/voyager-1/) and [Voyager 2](https://science.nasa.gov/mission/voyager/voyager-2/) pages again on September 20, 2026, in development. The listening station, dispatch puzzle, characters and recordings are fictional. NASA does not endorse this project. Source receipts show their original extraction time, including when the development build reuses a recent public receipt.
...
The September 20 interface uses a wider two-desk investigation surface, full-size evidence and a single explicit next action. Team details, recovered story, optional archive tools and case history stay behind expandable sections. Replay is labelled "Replay with new codes": the authored story repeats, while a published validated pack may change the opening assignment and the cipher and relay values vary per room. Firecrawl verifies the two fixed NASA sources; it does not generate testimony or endings.
